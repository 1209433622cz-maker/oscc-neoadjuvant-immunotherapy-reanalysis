...
GSE195832_MODELS = (
    ...
    / "03_rebuild"
    / "validation"
    / "GSE195832_bulk_locked_family"
    / "GSE195832_FAMILY_RESPONSE_MODELS.csv"
)
GSE195832_GATES = (
    WORKSPACE
    / "03_rebuild"
    / "validation"
    / "GSE195832_bulk_locked_family"
    / "GSE195832_VALIDATION_GATES.csv"
)
...
def draw_cohort_matrix(ax: plt.Axes) -> None:
    rows = [
        ("GSE200996", "Discovery", "partial"),
        ("GSE281729", "Timing-sensitive", "ready"),
        ("GSE195832", "Eligible null", "independent"),
        ("GSE179730", "Exact null", "pending"),
        ("GSE301741", "Provisional null", "pending"),
        ("GSE123813", "Cross-disease", "cross"),
    ]
    colors = {
        "ready": "#1f1f1f",
        "partial": "#555555",
    ...
    x_positions = np.linspace(0.07, 0.93, len(rows))
    ax.plot([0.07, 0.93], [0.60, 0.60], color="#bdbdbd", lw=0.7, transform=ax.transAxes, zorder=1)
    for x, (cohort, role, status) in zip(x_positions, rows):
        ax.scatter(
            x,
            0.60,
            s=52,
        ...
        ax.text(x, 0.35, cohort, ha="center", va="center", fontsize=6.5, fontweight="bold", transform=ax.transAxes)
        ax.text(x, 0.13, role, ha="center", va="center", fontsize=5.5, color="#444444", transform=ax.transAxes)
    ax.text(-0.035, 1.02, "a", fontsize=8, fontweight="bold", va="top", transform=ax.transAxes)
...
def draw_gse281729(ax: plt.Axes, df: pd.DataFrame) -> None:
    ...
    ax.text(-0.12, 1.04, "b", fontsize=8, fontweight="bold", va="top", transform=ax.transAxes)
...
def draw_gse179730(ax: plt.Axes, df: pd.DataFrame) -> None:
    ...
    ax.text(-0.20, 1.04, "c", fontsize=8, fontweight="bold", va="top", transform=ax.transAxes)
    ...
    ax.set_xticks([-0.25, 0.00, 0.25, 0.50, 0.75])
    ax.spines["top"].set_visible(False)
    ax.spines["right"].set_visible(False)
    ax.grid(axis="x", color="#dddddd", lw=0.4)


def draw_inference_matrix(ax: plt.Axes) -> None:
    ax.set_xlim(0, 1)
    ax.set_ylim(0, 1)
    ax.text(-0.06, 1.04, "d", fontsize=8, fontweight="bold", va="top", transform=ax.transAxes)
    ax.text(0.02, 1.04, "Cross-cohort inference gates", fontsize=8, fontweight="bold", va="top", transform=ax.transAxes)
    rows = [
        ("GSE281729", ("yes", "yes", "yes"), "Global-PC sensitive"),
        ("GSE195832", ("yes", "yes", "yes"), "Null; opposite"),
        ("GSE179730", ("yes", "yes", "yes"), "Exact null"),
        ("GSE301741", ("yes", "limited", "yes"), "T-cell null; myeloid NE"),
        ("GSE123813", ("no", "no", "yes"), "Cross-disease boundary"),
    ]
    x_cols = [0.40, 0.55, 0.69]
    for x, label in zip(x_cols, ["HNSCC", "Resp.", "Pair"]):
        ax.text(x, 0.86, label, ha="center", va="center", fontsize=5.2, fontweight="bold")
    ax.text(0.79, 0.86, "Inference", ha="left", va="center", fontsize=5.5, fontweight="bold")
    colors = {"yes": "#0072B2", "limited": "#E69F00", "no": "white"}
    edge = {"yes": "#0072B2", "limited": "#E69F00", "no": "#8f8f8f"}
    for idx, (cohort, gates, inference) in enumerate(rows):
        y = 0.72 - idx * 0.145
        ax.text(0.02, y, cohort, ha="left", va="center", fontsize=5.8, fontweight="bold")
        for x, gate in zip(x_cols, gates):
            ax.add_patch(
                Rectangle(
                    (x - 0.018, y - 0.018),
                    0.036,
        ...
        ax.text(0.79, y, inference, ha="left", va="center", fontsize=5.2, color="#333333")
    ax.text(
        0.02,
        0.02,
        "Blue, eligible; orange, provisional; open, not eligible. NE, not estimable.",
        fontsize=5.1,
        color="#555555",
        va="bottom",
    )
...
def main() -> None:
    ...
    gse195832_models = pd.read_csv(GSE195832_MODELS)
    gse195832_models = gse195832_models[
        gse195832_models["scoring_method"].eq("z_score")
        & gse195832_models["score_variant"].eq("module_mean_16")
    ].copy()
    gse195832_gates = pd.read_csv(GSE195832_GATES)
    ...
    gse195832_models.to_csv(
        SRC_DIR / "Figure3_gse195832_models_source.csv", index=False
    )
    gse195832_gates.to_csv(
        SRC_DIR / "Figure3_gse195832_gates_source.csv", index=False
    )
