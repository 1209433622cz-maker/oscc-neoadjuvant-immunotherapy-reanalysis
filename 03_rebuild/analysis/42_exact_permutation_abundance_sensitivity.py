...
def main() -> None:
    ...
    for ax, analysis, panel in zip(
        ...
        ax.invert_yaxis()
        ax.set_title(labels[analysis], loc="left", weight="bold", fontsize=11)
        ax.set_xlabel("Post-pre logit-abundance effect")
        ax.grid(axis="x", color="#E5E5E5", lw=0.6)
        ax.spines[["top", "right"]].set_visible(False)
        ax.text(-0.14, 1.07, panel, transform=ax.transAxes, weight="bold", fontsize=15)
        ax.margins(x=0.15)
    ...
    ax.set_title("Exact null distributions", loc="left", weight="bold", fontsize=11)
    ax.grid(axis="x", color="#E5E5E5", lw=0.6)
    ax.spines[["top", "right"]].set_visible(False)
    ax.text(-0.14, 1.07, "d", transform=ax.transAxes, weight="bold", fontsize=15)

    fig.suptitle(
        "Patient-label permutation constrains paired abundance inference",
        x=0.07,
        ha="left",
        fontsize=14,
        weight="bold",
    )
    fig.tight_layout(rect=(0, 0, 1, 0.96), h_pad=2.0, w_pad=4.8)
