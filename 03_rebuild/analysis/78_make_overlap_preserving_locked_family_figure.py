...
OVERLAP_DIR = (
    WORKSPACE / "03_rebuild" / "validation" / "locked_family_overlap_null"
)
TEST_PATH = OVERLAP_DIR / "OVERLAP_PRESERVING_FAMILY_TESTS.csv"
RANDOM_PATH = OVERLAP_DIR / "OVERLAP_PRESERVING_MATCHED_RANDOM_EFFECTS.csv"
AUDIT_PATH = OVERLAP_DIR / "OVERLAP_PRESERVING_NULL_AUDIT.csv"
...
def draw_random_null(
    ...
    ax.set_title(
        f"GSE281729: {label} overlap-preserving null",
        loc="left",
        fontsize=8,
        fontweight="bold",
        pad=5,
...
def main() -> None:
    ...
    tests.to_csv(
        SOURCE_DIR / "ExtendedData11_locked_family_tests_source.csv",
        index=False,
    )
    random_effects.to_csv(
        SOURCE_DIR
        / "ExtendedData11_overlap_preserving_random_effects_source.csv",
        index=False,
    )
    exact.to_csv(
        SOURCE_DIR / "ExtendedData11_gse179730_exact_null_source.csv",
        index=False,
    )
    audit.to_csv(
        SOURCE_DIR / "ExtendedData11_overlap_preserving_null_audit_source.csv",
        index=False,
    )
